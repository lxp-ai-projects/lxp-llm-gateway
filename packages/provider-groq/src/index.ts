...
export class GroqProviderAdapter implements LlmProviderAdapter {
  ...
  constructor(
    ...
    requestTimeoutMs = Number(process.env.GROQ_REQUEST_TIMEOUT_MS ?? '90000'),
  ) {
    this.baseUrl = baseUrl.replace(/\/$/, '');
    this.requestTimeoutMs = requestTimeoutMs;
  }

  readonly providerId: LlmProviderAdapter['providerId'] = 'groq';
  ...
  async listModels(
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Groq model listing failed with status ${response.status}: ${errorText}`,
      );
    }
  ...
  async chatStream(
    ...
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(
        `Groq streaming request failed with status ${response.status}: ${errorText}`,
      );
    }
