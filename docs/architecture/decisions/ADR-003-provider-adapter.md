...
- image-specific provider details such as xAI's `/v1/images/generations` and `/v1/images/edits` endpoints, or Google's native Gemini `generateContent` image flow, must remain inside provider packages, not `gateway-api`
