...
export class ConversationTransferController {
  constructor(private readonly conversationTransferService: ConversationTransferService) {}
  ...
  exportConversation(
    ...
    const exported = this.conversationTransferService.exportConversation(dto.conversation);
  ...
  exportConversationArchive(
    ...
    const exported = this.conversationTransferService.exportConversationArchive(dto.conversations);
  ...
  importConversationFile(@UploadedFile() file?: { originalname: string; buffer: Buffer }) {
    if (!file) {
